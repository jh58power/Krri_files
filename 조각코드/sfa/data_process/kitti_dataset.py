...
class KittiDataset(Dataset):
    def __init__(self, configs, mode='train', lidar_aug=None, hflip_prob=None, num_samples=None):
        ...
        self.lidar_dir = os.path.join(self.dataset_dir, sub_folder, "velodyne_2021_final")
        self.calib_dir = os.path.join(self.dataset_dir, sub_folder, "calib")
        self.label_dir = os.path.join(self.dataset_dir, sub_folder, "label_2")
        split_txt_path = os.path.join(self.dataset_dir, 'ImageSets', '{}.txt'.format(mode))
        self.sample_id_list = [x.strip() for x in open(split_txt_path).readlines()]
    ...
    def get_lidar(self, file):
        lidar_file = os.path.join(self.lidar_dir, '{}.pcd'.format(file))
        pcd = o3d.t.io.read_point_cloud(lidar_file,format='pcd')
        pcdXYZ = np.asarray(pcd.point['positions'].numpy(),dtype=np.float32)
        
        #--------------------X,Y 90도 회전-------------------------------------
        # R = self.rotMatrix(-1.5708) #0.785398 = 45도  1.5708 = 90도
        # pcdXYZ[:,[0,1]] = np.dot(R, pcdXYZ[:,[0,1]].T).T
        
        #--------------------------------------------------------------------
        # R = self.rotMatrix(-0.785398) #0.785398 = 45도  1.5708 = 90도
        # R2 = self.rotMatrix(1.5708)
        # pcdXYZ[:,[0,2]] = np.dot(R, pcdXYZ[:,[0,2]].T).T
        # pcdXYZ[:,[1,2]] = np.dot(R2, pcdXYZ[:,[1,2]].T).T
        ...
        pcdInten = np.asarray(pcd.point['intensities'].numpy(),dtype=np.float32)
        
        return np.concatenate((pcdXYZ,pcdInten),axis=1).reshape(-1, 4)
    ...
    def get_label(self, filename):
        ...
        label_path = os.path.join(self.label_dir,"{}.json".format(filename))
        ...
        for obj in json.load(json_file):
            ...
            # h,w,l = obj["psr"]["3D_Dimenstion"]["height"],obj["psr"]["3D_Dimenstion"]["length"],obj["psr"]["3D_Dimenstion"]["width"]
            ...
            #--------------2020 Data Label 회전변환 ------------------
            # R = self.rotMatrix(0.724440610153788) # 약 41도
            # R2 = self.rotMatrix(1.5708*2)#0.785398 = 45도  1.5708 = 90도     
                
            # y,z = np.dot(R,np.array([y,z]).T).T
            # x,y = np.dot(R2,np.array([x,y]).T).T
            # ry = ry + 1.5708
            #-------------------------------------------------------
    ...
    def draw_img_with_label(self, filename):
        sample_id = self.sample_id_list[filename]
...
if __name__ == '__main__':
    ...
    for idx in range(len(dataset)):
        ...
        # labels = dataset.load_img_with_targets(idx)
