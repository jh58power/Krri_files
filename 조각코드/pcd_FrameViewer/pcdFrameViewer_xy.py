...
# def find_nearest(path, num):
#     file_list = os.listdir(path)
#     i = 0
#     for file in file_list:
#         file_list[i] = abs(int(file[:-4]) - int(num))
#         i += 1
#     idx = file_list.index(min(file_list))
#     file_list = os.listdir(path)
...
while i <= len(img_list):
    ...
    origin_pcd = pcd_path+'/'+ pcd_list[i]
    # copy_pcd = pcd_output_path +'/'+ near_pcd
    
    pc_image = pcd_projection(origin_pcd)
    pc_image_rot = cv2.rotate(pc_image,cv2.ROTATE_180)
    
    cv2.imshow('PointCloud',pc_image_rot)
